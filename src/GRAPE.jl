...
"""
function _fom_and_gradient_GRAPE!(A::T, B, control_array, n_timeslices, duration, n_controls, gradient, fwd_state_store, bwd_costate_store, propagators, X_init, X_target, evolve_store, system_type) where T
    ...
    for t = reverse(1:n_timeslices)
        evolve_func!(system_type, t, fwd_state_store, bwd_costate_store, propagators, evolve_store, forward = false)
    end
    ...
    for c = 1:n_controls
        for t = 1:n_timeslices
            @views gradient[c, t] = grad_func!(system_type, t, dt, B[c], fwd_state_store, bwd_costate_store, propagators, evolve_store)
        end
    end
...
"""
function _fom_and_gradient_sGRAPE(A::T, B, control_array, n_timeslices, duration, n_controls, gradient, fwd_state_store, bwd_costate_store, X_init, X_target, system_type) where T
    ...
    for t = 1:n_timeslices
        fwd_state_store[t+1] = evolve_func(system_type, t, fwd_state_store, bwd_costate_store, propagators, forward = true)
    end
    # backward evolution of costates
    for t = reverse(1:n_timeslices)
        bwd_costate_store[t] = evolve_func(system_type, t, fwd_state_store, bwd_costate_store, propagators, forward = false)
    end
    # update the gradient array
    for c = 1:n_controls
        for t = 1:n_timeslices
            gradient[c, t] = grad_func(system_type, t, dt, B[c], fwd_state_store, bwd_costate_store, propagators)
...
"""
function evolve_func(system_type::UnitaryGate, t, state, costate, propagator; forward = true)
    if forward
...
"""
function evolve_func(system_type::Union{StateTransfer,CoherenceTransfer}, t, state, costate, propagator ;forward = true)
    if forward
...
"""
function evolve_func!(system_type::UnitaryGate, t, state, costate, propagator, store; forward = true)
    if forward
...
"""
function evolve_func!(system_type::Union{StateTransfer,CoherenceTransfer}, t, state, costate, propagator, store; forward = true)
    if forward
...
"""
function grad_func!(system_type::UnitaryGate, t, dt, B, state, costate, props, store)::Float64
    @views mul!(store, state[t]', costate[t])
    @views 2.0 * real((1.0im * dt) .* tr(costate[t]' * B * state[t]) * tr(store))
end

function grad_func!(system_type::Union{StateTransfer,CoherenceTransfer}, t, dt, B, state, costate, props, store)::Float64
...
function grad_func(system_type::Union{StateTransfer,CoherenceTransfer}, t, dt, B, state, costate, props)
    real(tr(

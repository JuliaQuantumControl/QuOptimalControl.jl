...
"""
function fom_func(prob::UnitaryProblem, t, k, state, costate, props)::Float64
    @views tr(costate[t, k]' * state[t, k]) * tr(state[t, k]' * costate[t, k])
end

function fom_func(prob::Union{StateTransferProblem,OpenSystemCoherenceTransfer}, t, k, state, costate, props)::Float64
...
function fom_func(prob::UnitaryProblem, t, state, costate, props)::Float64
    @views tr(state[t]' * costate[t]) * tr(state[t]' * costate[t])
end

function fom_func(prob::Union{StateTransferProblem,OpenSystemCoherenceTransfer}, t, state, costate, props)::Float64
